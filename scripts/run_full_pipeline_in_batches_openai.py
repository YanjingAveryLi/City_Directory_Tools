...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run the OpenAI-only pipeline in repeated batches until targets=0.")
    ap.add_argument("--base_dir", default=".", help="Project root")
    ap.add_argument("--roots", default="", help="Comma-separated roots. Empty means auto-detect *_txt under base_dir.")
    ...
    ap.add_argument("--org_model", default="gpt-4.1-nano")
    ap.add_argument("--name_extraction_model", default="gpt-4.1-nano")
    ap.add_argument("--category_model", default="gpt-4.1-nano")
    ap.add_argument("--category_refine_model", default="gpt-5.4")
    args = ap.parse_args()
